...
impl Field {
    ...
    pub fn encode(&self, prost_path: &Path, ident: TokenStream) -> TokenStream {
        ...
        let ident_value = if let Some(wrapper) = &self.key_wrapper {
            let type_name = &wrapper.type_name;
            let raw_map_ref = self.map_ty.raw_map_ref_method();
            quote! {#type_name::#raw_map_ref(&#ident)}
        } else {
            quote! {&#ident}
        };
        match &self.value_ty {
    ...
    pub fn merge(&self, prost_path: &Path, ident: TokenStream) -> TokenStream {
        ...
        let ident_value = if let Some(wrapper) = &self.key_wrapper {
            let type_name = &wrapper.type_name;
            let raw_map_mut_ref = self.map_ty.raw_map_mut_ref_method();
            quote! {#type_name::#raw_map_mut_ref(&mut #ident)}
        } else {
            quote! {&mut #ident}
        };
        match &self.value_ty {
            ...
            ValueTy::Scalar(value_ty) => {
                ...
                quote!(#prost_path::encoding::#module::merge(#km, #vm, #ident_value, buf, ctx))
            }
            ValueTy::Message => quote! {
    ...
    pub fn encoded_len(&self, prost_path: &Path, ident: TokenStream) -> TokenStream {
        ...
        match &self.value_ty {
            ...
            ValueTy::Scalar(value_ty) => {
                let val_mod = value_ty.module();
                let vl = quote!(#prost_path::encoding::#val_mod::encoded_len);
                quote!(#prost_path::encoding::#module::encoded_len(#kl, #vl, #tag, #ident_value))
            }
            ValueTy::Message => quote! {
    ...
    pub fn debug(&self, prost_path: &Path, wrapper_name: TokenStream) -> TokenStream {
        ...
        let key = if let Some(wrapper) = &self.key_wrapper {
            ...
            self.key_ty.rust_type(prost_path)
        };
